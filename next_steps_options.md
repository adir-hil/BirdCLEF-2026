...
| 1 | SED Attention Model | HIGH | Medium | Yes | DONE (no gain) | Switch from simple global-average pooling to Sound Event Detection model with attention pooling. Already built in `src/model.py` (`BirdCLEFSED`). | Scored 0.855 vs 0.856. 5-second windows too short for attention to help. Architecture is not the bottleneck. |
...
3. ~~**Option 1** (SED model) — scored 0.855, no improvement, architecture not the bottleneck~~
